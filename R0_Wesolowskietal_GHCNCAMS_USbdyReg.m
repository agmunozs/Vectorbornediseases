%%Wesolowski et al (PNAS, 2015) R0 model 
...
for iy=1:nlatp
      for ix=1:nlonp
        for it=1:ndat2
            R0(iy,ix,it)=sqrt((a(iy,ix,it)^2*bc(iy,ix,it)*(e2a(iy,ix,it)*MDR(iy,ix,it)/(mu(iy,ix,it))^2)*exp((-mu(iy,ix,it)/(PDR(iy,ix,it)+ec))))/(mu(iy,ix,it)));
        end
      end
end 
...
save -v7.3 R0_Wesolowski_GHCNCAMS.mat R0 X Y 
disp('Vector model has been computed and saved in R0_Wesolowski_GHCNCAMS.mat');
...
t=0:815;
%Open the file
ncid = netcdf.create(['./R0_Wesolowskietal_GHCN-CAMS.nc'],'NC_WRITE');
...
disp('Nectdf file produced: R0_Wesolowskietal_GHCN-CAMS.nc');